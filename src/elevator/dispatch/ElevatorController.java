...
public final class ElevatorController {
    ...
    private final Map<Integer, ElevatorCar> carsById;
    private final SchedulingStrategy schedulingStrategy;

    public ElevatorController(List<ElevatorCar> cars, SchedulingStrategy schedulingStrategy) {
        this.cars = List.copyOf(cars);
        this.carsById = cars.stream().collect(Collectors.toUnmodifiableMap(ElevatorCar::id, Function.identity()));
    ...
    public void selectDestination(int elevatorId, int destinationFloor) {
        car(elevatorId).addDestinationStop(destinationFloor);
    }

    public void setMaintenanceMode(int elevatorId, boolean enabled) {
        car(elevatorId).setMaintenance(enabled);
    }

    public void emergencyStop(int elevatorId) {
        car(elevatorId).emergencyStop();
    }

    private ElevatorCar car(int id) {
        ElevatorCar car = carsById.get(id);
        if (car == null) throw new IllegalArgumentException("Unknown elevator id: " + id);
        return car;
    }
}
