...
public final class NearestEligibleCarStrategy implements SchedulingStrategy {
    ...
    private int score(ElevatorCar car, HallRequest request) {
        ...
        boolean onTheWay = car.direction() == request.direction() && ((request.direction() == Direction.UP && car.currentFloor() <= request.floor()) || (request.direction() == Direction.DOWN && car.currentFloor() >= request.floor()));
